...
function debug(...messages: any[]) {
  if (DEBUG) {
    console.log('[SIGN-UP CALLBACK]', ...messages);
  }
}
    ...
    async function processCallback() {
      try {
        ...
        if (params.get('error')) {
          ...
          // Redirect back to sign-up after displaying the error
          setTimeout(() => {
            router.push("/sign-up");
          }, 3000);
          return;
        ...
        // Default fallback
        debug("No complete status found, returning to sign-up");
        setProcessing(false);
        setTimeout(() => {
          router.push("/sign-up");
        }, 1500);
  ...
  return (
    <div className="flex justify-center items-center min-h-screen">
      <div className="text-center">
        <h1 className="text-xl font-semibold mb-4">Completing sign up...</h1>
        {error ? (
          <div className="text-red-500 mb-4 p-4 bg-red-50 rounded-lg">
            <p className="font-semibold">Error occurred:</p>
            <p>{error}</p>
            <p className="mt-2 text-gray-600">Redirecting you back to the sign-up page.</p>
          </div>
        ) : (
          <>
